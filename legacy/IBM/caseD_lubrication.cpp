...
//    CellStretch<T, DESCRIPTOR> cellStretch(RBCField, stretchForceScalar, 0.1);
    ...
    if (not checkpointer.wasCheckpointed()) {
        ...
        cellsOrigin.push_back( hemo::Array<T,3>(nx*0.5 - 5*radius, ny*0.5, nz*0.5) );
        cellsOrigin.push_back( hemo::Array<T,3>(nx*0.5 + 5*radius, ny*0.5, nz*0.5) );
//        cellsOrigin.push_back( hemo::Array<T,3>(nx*0.5 - 2*radius, ny*0.5, nz*0.5) );
//        cellsOrigin.push_back( hemo::Array<T,3>(nx*0.5 + 2*radius, ny*0.5, nz*0.5) );
...
//#endif
    ...
    const hemo::Array<T,3> headOnForce(stretchForceScalar*0.5, 0.0, 0.0);
    const hemo::Array<T,3> realignForceY(0.0, stretchForceScalar, 0.0);
    const hemo::Array<T,3> realignForceZ(0.0, 0.0, stretchForceScalar);
    std::vector<hemo::Array<T,3> > forcesToApply(2);
    std::vector<plint> RBCellIds(2);
    RBCellIds[0] = 0; RBCellIds[1] = 1;
    T DeltaYNorm = 0;
    T DeltaZNorm = 0;
