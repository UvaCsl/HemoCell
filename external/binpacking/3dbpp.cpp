 ...
 *             solution is found; otherwise it terminate_3dbpps after timelimit
...
#define MAXITER_3dbpp      1000  /* max iterations in heuristic onebin_robot */
...
#define GENERAL_3dbpp     0     /* packing type */
#define ROBOT       1

#define LEFT_3dbpp   0          /* relative placements */
#define RIGHT_3dbpp  1
#define UNDER_3dbpp  2
#define ABOVE_3dbpp  3
#define FRONT_3dbpp  4
#define BEHIND_3dbpp 5
#define UNDEF_3dbpp  6
#define RELMAX 8

#define STACKDEPTH_3dbpp (MAXBOXES*MAXBOXES*RELMAX)

#define VOL_3dbpp(i)            ((i)->w * (ptype) (i)->h * (i)->d)
...
typedef struct irec {
  ...
  boolean_3dbpp  k;            /* is the box chosen?                    */
...
typedef struct {
  ...
  boolean_3dbpp  packtype;     /* packing type: GENERAL_3dbpp or ROBOT        */
  ...
  boolean_3dbpp  *closed;      /* for each bin indicator whether closed */
...
typedef struct {
  ...
  boolean_3dbpp domain;        /* domain of the two boxes               */
} domainpair;
...
/* boolean_3dbpp variable to indicate time-out situation */
boolean_3dbpp stopped; 

/* counter used to ensure that 1D BPP at most performs MAXBPP iterations */
int bpiterat;

/* boolean_3dbpp variables to indicate when 1D packing algorithm should terminate_3dbpp */
boolean_3dbpp feasible, terminate_3dbpp;

/* stack of domain pairs */
domainpair domstack[STACKDEPTH_3dbpp];
...
boolean_3dbpp checksol(allinfo *a, box *f, box *l)
{
  box *i, *j, *m;
  boolean_3dbpp stillboxes, foundextreme, extreme;
  int b;
  boolean_3dbpp isok=1;
  for (i = f, m = l+1; i != m; i++) { 
    if (!i->k) continue;  /* box currently not chosen */
    for (j = f; j != m; j++) {
      if (i == j) continue;
      if (i->no == j->no) {
        printf("(3dbpp): duplicated box %d\n", i->no);
        isok=0;
      } 
      if (!j->k) continue;
	  ...
	  (i->z + i->d > j->z) && (j->z + j->d > i->z)) {
	     printf("(3dbpp): overlap box %d,%d: [%d,%d,%d] [%d,%d,%d]\n",
  ...
  if (a->packtype != ROBOT) return isok;
  ...
  for (b = 1; b <= a->z; b++) {
    for (;;) {
      ...
      for (i = f; i <= m; i++) { 
        ...
        if (extreme) { i->k = -1; foundextreme = 1; }
      }
    ...
    if (!foundextreme) {
      ...
      printf("(3dbpp): not robot packable\n");
    }
    /* restore k values */
...
void savesol(allinfo *a, box *f, box *l, ntype z)
{
  ...
  if (z >= a->z) printf("(3dbpp): not improved\n");
  for (i = f, m = l+1; i != m; i++) {
    if ((1 <= i->bno) && (i->bno <= z)) continue; 
    printf("(3dbpp): illegal bin %d, box %d \n", i->bno, i->no); 
  }
  ...
  for (i = a->fopt, m = a->lopt+1; i != m; i++) i->k = 1;
  if (DIF_3dbpp(a->fopt,k-1) != a->n) printf("(3dbpp): not correct amount of boxes \n");
...
void isortincr(int *f, int *l)
{
  ...
  d = l - f + 1;
  if (d < 1) printf("(3dbpp): negative interval in isortincr \n");
  if (d == 1) return;
  m = f + d / 2; if (*f > *m) SWAPINT_3dbpp(f, m);
  if (d > 2) { if (*m > *l) { SWAPINT_3dbpp(m, l); if (*f > *m) SWAPINT_3dbpp(f, m); } }
  if (d <= 3) return;
  mi = *m; i = f; j = l;
  for (;;) {
    do i++; while (*i < mi);
    do j--; while (*j > mi);
    if (i > j) break; else SWAPINT_3dbpp(i, j);
  }
...
void isortdecr(int *f, int *l)
{
  ...
  d = l - f + 1;
  if (d < 1) printf("(3dbpp): negative interval in isortdecr \n");
  if (d == 1) return;
  m = f + d / 2; if (*f < *m) SWAPINT_3dbpp(f, m);
  if (d > 2) { if (*m < *l) { SWAPINT_3dbpp(m, l); if (*f < *m) SWAPINT_3dbpp(f, m); } }
  if (d <= 3) return;
...
void psortdecr(point *f, point *l)
{
  ...
  d = l - f + 1; 
  if (d <= 1) return;
  m = f + d / 2; if (DF_3dbpp(*f,*m)<0) SWAPP_3dbpp(f,m); 
  if (d == 2) return;
  if (DF_3dbpp(*m,*l)<0) { SWAPP_3dbpp(m,l); if (DF_3dbpp(*f,*m)<0) SWAPP_3dbpp(f,m); }
  if (d <= 3) return;
  mi = *m; i = f; j = l;
  for (;;) {
    do i++; while (DF_3dbpp(*i,mi) > 0);
    do j--; while (DF_3dbpp(*j,mi) < 0);
    if (i > j) break; else SWAPP_3dbpp(i, j);
  }
...
int bound_one_x(allinfo *a, box *f, box *l)
{
  ...
  for (pp = plist; *pp != 0; pp++) {
    ...
    if (alpha < 0) alpha = 0;
    if (beta  < 0) beta  = 0;
    lb_one = j1 + j2 + MAXIMUM_3dbpp(alpha, beta);
...
void onelayer(allinfo *a, box *f, box *m, box *l, int d)
{
  ...
  qsort(f, DIF_3dbpp(f,m), sizeof(box), (funcptr) hcomp);
  r = a->W;
...
box *remboxes(box *f, box *l, itype *depth)
{
  ...
  for (i = f, j = l, d = 0; i != j+1; ) {
    if (i->k) { if (i->d > d) d = i->d; i++; } else { SWAP_3dbpp(i,j); j--; }
  }
  *depth = d;
  return i;
}
...
/* algorithm terminate_3dbpps when MAXBPP iterations have been performed, */
...
void dfirst_heuristic(allinfo *a)
{
  ...
  for (j = a->fbox, m = a->lbox+1; j != m; j++) {
    j->bno = j->x = j->y = j->z = 0; j->k = 0;
  }

  /* fill layer one by one */
  for (f = a->fbox, l = a->lbox, h = 0; ; h++) {
    n = DIF_3dbpp(f,l); if (n == 0) break;
    ...
    t[h].z = 0; t[h].lno = h+1;  /* this ensures fes. solution if terminate_3dbpp */
  }
...
/* **********************************************************************
   **********************************************************************
                    fill one 3D bin using GENERAL_3dbpp packing
   **********************************************************************
   ********************************************************************** */
...
void modifyandpush(int i, int j, int rel, boolean_3dbpp dom)
{
  dompos->i = i;
  dompos->j = j;
  dompos->domain = dom;
  ...
  dompos++; if (dompos == domend) printf("(3dbpp): stack filled\n");
}
...
void popdomains(domainpair *pos)
{
  for (; dompos != pos; ) {
    dompos--;
    if (dompos->domain) {
      domain[dompos->i][dompos->j][dompos->relation] = 1;
    } else {
 ...
 * the algorithm terminate_3dbpps with 0. Otherwise 1 is returned, saying
 * that a feasible packing exists which respects the current relations.
 */

boolean_3dbpp findcoordinates(allinfo *a, int n, box *f)
{
  register box *g, *h;
  register int sum;
  int i, j, k, W, H, D;
  boolean_3dbpp changed;
  char *dom, *relij;
  domset *domij;
      ...
      for ( ; j < n; j++, relij++) {
        ...
        switch (*relij) {
          ...
          case LEFT_3dbpp  : 
            sum = g->x + g->w;
            if (h->x < sum) {
              h->x = sum; changed = 1; if (sum + h->w > W) return 0;
            }
            break;
          case RIGHT_3dbpp : 
            sum = h->x + h->w;
            if (g->x < sum) {
              g->x = sum; changed = 1; if (sum + g->w > W) return 0;
            }
            break;
          case UNDER_3dbpp : 
            sum = g->y + g->h;
            if (h->y < sum) {
              h->y = sum; changed = 1; if (sum + h->h > H) return 0;
            }
            break;
          case ABOVE_3dbpp : 
            sum = h->y + h->h;
            if (g->y < sum) {
              g->y = sum; changed = 1; if (sum + g->h > H) return 0;
            }
            break;
          case FRONT_3dbpp : 
            sum = g->z + g->d;
            if (h->z < sum) {
              h->z = sum; changed = 1; if (sum + h->d > D) return 0;
            }
            break;
          case BEHIND_3dbpp: 
            sum = h->z + h->d;
            if (g->z < sum) {
              g->z = sum; changed = 1; if (sum + g->d > D) return 0;
            }
            break;
        }
      }
...
void checkdomain(allinfo *a, int i, int j, int n, box *f, int value)
{
  ...
  relation[i][j] = value;
  if (findcoordinates(a, n, f) == 0) {
    modifyandpush(i, j, value, 1);
  } 
}
...
boolean_3dbpp reducedomain(allinfo *a, int n, box *f)
{
  ...
  for (i = 0; i < n-1; i++) {
    for (j = i+1; j < n-1; j++) {
      if (relation[i][j] == UNDEF_3dbpp) {
        checkdomain(a, i, j, n, f, LEFT_3dbpp);
        checkdomain(a, i, j, n, f, RIGHT_3dbpp);
        checkdomain(a, i, j, n, f, UNDER_3dbpp);
        checkdomain(a, i, j, n, f, ABOVE_3dbpp);
        checkdomain(a, i, j, n, f, FRONT_3dbpp);
        checkdomain(a, i, j, n, f, BEHIND_3dbpp);
        relation[i][j] = UNDEF_3dbpp;
        for (k = LEFT_3dbpp, l = 0; k < UNDEF_3dbpp; k++) {
          if (domain[i][j][k]) { l++; m = k; }
        }
        if (l == 0) return 0;
        if (l == 1) { modifyandpush(i, j, m, 0); }
      }
    } 
  }
  return 1;
 ...
 * has an associated relation with domain LEFT_3dbpp, RIGHT_3dbpp, UNDER_3dbpp, ABOVE_3dbpp, FRONT_3dbpp, 
 * BEHIND_3dbpp. In each iteration of the algorithm a pair of boxes "i" and "j"
...
void recpack(allinfo *a, int i, int j, int n, box *f, int rel)
{
  ...
  if (stopped) return;
  a->iter3d++;
  if ((a->iter3d == a->maxiter) && (a->maxiter != 0)) terminate_3dbpp = 1;
  a->subiterat++; 
  ...
  if (terminate_3dbpp) return;

  relation[i][j] = rel;

  for (i1 = 0, j1 = 0; i1 != i && j1 != j; ) {
    i1++; if (i1 >= j1) { i1 = 0; j1++; }
    if (relation[i1][j1] == UNDEF_3dbpp) printf("(3dbpp): relation error %d %d\n", i1, j1);
  }  
  ...
  if (feas) {
    ...
    bblevel++;
    rel = relation[i][j];
    if (domain[i][j][LEFT_3dbpp ]) recpack(a, i, j, n, f, LEFT_3dbpp);
    if (domain[i][j][RIGHT_3dbpp]) recpack(a, i, j, n, f, RIGHT_3dbpp);
    if (domain[i][j][UNDER_3dbpp]) recpack(a, i, j, n, f, UNDER_3dbpp);
    if (domain[i][j][ABOVE_3dbpp]) recpack(a, i, j, n, f, ABOVE_3dbpp);
    if (domain[i][j][FRONT_3dbpp]) recpack(a, i, j, n, f, FRONT_3dbpp);
    if (domain[i][j][BEHIND_3dbpp])recpack(a, i, j, n, f, BEHIND_3dbpp);
    relation[i][j] = rel;
    bblevel--;
  }
  popdomains(pos);
 ...
 * each pair of boxes initially has an associated relation with domain LEFT_3dbpp, 
 * RIGHT_3dbpp, UNDER_3dbpp, ABOVE_3dbpp, FRONT_3dbpp, BEHIND_3dbpp. Then the recursive algorithm "recpack" 
...
boolean_3dbpp general_pack(allinfo *a, box *f, box *l)
{
  register int i, j, k, n;

  dompos = domstack;
  domend = domstack + STACKDEPTH_3dbpp;
  feasible = 0;
  terminate_3dbpp = 0;
  bblevel = 1;
  n = l-f+1;
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      relation[i][j] = UNDEF_3dbpp;
      for (k = LEFT_3dbpp; k < UNDEF_3dbpp; k++) {
        domain[i][j][k] = 1;
      }
    }
  }
  domain[0][1][RIGHT_3dbpp ] = 0;
  domain[0][1][ABOVE_3dbpp ] = 0;
  domain[0][1][BEHIND_3dbpp] = 0;

  recpack(a, 0, 0, n, f, UNDEF_3dbpp); 
  return feasible;
}
 ...
 * packing. If "fast" is 1, the problem is only solved heuristically,
 * hence if the algorithm returns 0 we cannot rule out the posibility
 * of packing all boxes into the bin.
 */

boolean_3dbpp onebin_general(allinfo *a, box *f, box *l, boolean_3dbpp fast)
{
  boolean_3dbpp solution;
  double t1, t2;
  ...
  a->iter3d = 0;
  a->maxiter = (fast ? MAXITER_3dbpp : 0); /* limited or infinitly many */
  a->exactcall++;
...
void branch(allinfo *a, box *f, box *l, int miss, stype fill)
{
  ...
  if (miss == 0) {
    ...
    memcpy(a->fsol, f, sizeof(box) * DIF_3dbpp(f,l));
    a->maxfill = a->BVOL; terminate_3dbpp = 1; a->miss = miss;
  } else {
	...
	for (i = f, d = 0; i != l+1; i++) {
	  ...
	  i->k = 1; i->x = s->x; i->y = s->y; i->z = s->z;
	  branch(a, f, l, miss - 1, fill + i->vol);
	  i->k = 0; i->x = i->y = i->z = 0; d++;
	  if (d == a->mcut) break; /* terminate_3dbpp after mcut branches */
	  if (terminate_3dbpp) break;
	}
      }
    }
  }
...
boolean_3dbpp onebin_robot(allinfo *a, box *f, box *l, boolean_3dbpp fast)
{
  box *i, *j, *m;
  stype vol;
  double t1, t2;
...
void mcut_heuristic(allinfo *a)
{
  ...
  for (b = 1; ; b++) {
    /* fill one bin */
    for (i = f; i <= l; i++) i->k = 0; /* box not chosen */
    a->iter3d = 0;
    a->maxfill = 0;
    a->miss = DIF_3dbpp(f,l);
    a->maxiter = 5*MAXITER_3dbpp;
    terminate_3dbpp = 0;
    n = DIF_3dbpp(f,l);
    a->mcut = 2;
    if (n < 15) a->mcut = 3;
    ...
    /* remove chosen boxes */
    for (i = f; i <= l;) if (i->k) { i->bno = b; SWAP_3dbpp(i,l); l--; } else i++;
...
boolean_3dbpp fits2(box *i, box *j, itype W, itype H, itype D)
{
  /* all coordinates are initialized to zero, so just adjust changes! */
  /* the 2-box solution is always guillotine cuttable */
  if (i->w + j->w <= W) { j->x = i->w; return 1; }
  if (i->h + j->h <= H) { j->y = i->h; return 1; }
  if (i->d + j->d <= D) { j->z = i->d; return 1; }
  return 0;
}


boolean_3dbpp fits2p(box *i, box *j, itype W, itype H, itype D)
{
  if (i->w + j->w <= W) return 1;
  if (i->h + j->h <= H) return 1;
  if (i->d + j->d <= D) return 1;
  return 0;
}


boolean_3dbpp fits3(box *i, box *j, box *k, itype W, itype H, itype D, int packtype)
{
  box *t;
  itype w, h, d, r;
...
boolean_3dbpp fitsm(allinfo *a, box *t, box *k, boolean_3dbpp fast)
{
  ...
  if (lb > 1) return 0;
  a->exfill++; fits = 0;
  if (a->packtype == GENERAL_3dbpp   ) fits = onebin_general(a, t, k, fast); 
...
boolean_3dbpp onebin_decision(allinfo *a, box *j, int bno)
{
  register box *i, *k, *m;
  box t[MAXBOXES];
  boolean_3dbpp fits;
  ...
  k++; *k = *j; k->x = k->y = k->z = 0; k->ref = j; k->k = 1; 

  size = DIF_3dbpp(t,k); 
  switch (size) {
    case 0: printf("(3dbpp): no boxes in onebin_decision \n");
    case 1: fits = 1; k->x = k->y = k->z = 0; break;
    case 2: fits = fits2(t, k, a->W, a->H, a->D); break;
    case 3: fits = fits3(t, t+1, k, a->W, a->H, a->D, a->packtype); break;
    default: fits = fitsm(a, t, k, 0); break;
  }
  if (size <= 3) {
    a->subiterat++; 
    if (a->subiterat == IUNIT) { 
  ...
  if (fits) {
    /* copy solution back */
    for (i = t, m = k+1; i != m; i++) {
      k = i->ref; k->x = i->x; k->y = i->y; k->z = i->z; k->k = 1;
    }
  }
  return fits;
}
...
boolean_3dbpp onebin_heuristic(allinfo *a, box *f, box *l)
{
  box *i, *m;
  boolean_3dbpp fits;
 
  for (i = f, m = l+1; i != m; i++) { i->x = i->y = i->z = 0; }
  switch (DIF_3dbpp(f,l)) {
    case 0: printf("(3dbpp): no boxes in onebin_heuristic \n");
    case 1: fits = 1; break;
    case 2: fits = fits2(f, l, a->W, a->H, a->D); break;
    case 3: fits = fits3(f, f+1, l, a->W, a->H, a->D, a->packtype); break;
    default: fits = fitsm(a, f, l, 1); break;
  }
  return fits;
}
...
boolean_3dbpp try_close(allinfo *a, box **curr, ntype bno,
                 box *oldf, box **oldl, box **oldlc, ntype *oldnoc, 
                 boolean_3dbpp *oldclosed, int level)
{
  register box *j, *m, *k, *r, *i, *s;
  register stype vol;
  box f[MAXBOXES];
  ntype h, n, b;
  boolean_3dbpp didclose, fits;

  if (level > MAXCLOSE) return 0;
  i = *curr; didclose = 0;
  for (b = 1; b <= bno; b++) {
    if (i > a->lbox) break;
    ...
    if (n == 0) printf("(3dbpp): bin with no boxes \n");
    if (vol < a->BVOL/2) continue;
    for (j = i, h = 0, m = a->lbox+1; j != m; j++) {
      if ((j->no > a->n) || (j->no < 1)) printf("(3dbpp): bad no \n");
    ...
    if (onebin_heuristic(a, f, k-1)) {
      if (!didclose) { /* take backup of table when first bin closed */
        memcpy(oldclosed, a->closed, sizeof(boolean_3dbpp)*(bno+1));
        memcpy(oldf, a->fbox, sizeof(box)*DIF_3dbpp(a->fbox,a->lbox));
        *oldl = a->lbox; *oldlc = a->lclosed; *oldnoc = a->noc;
      }
      a->closed[b] = 1; s = a->lclosed; didclose = 1;
      a->noc++; if (a->noc > a->maxclose) a->maxclose = a->noc;
      for (j = f; j != k; j++) { 
        r = j->ref; r->bno = b; r->k = 1; 
...
void free_close(allinfo *a, ntype bno, 
                box *oldf, box *oldl, box *oldlc, ntype oldnoc,
                boolean_3dbpp *oldclosed)
{
  a->lbox = oldl; a->lclosed = oldlc; a->noc = oldnoc;
  memcpy(a->fbox, oldf, sizeof(box)*DIF_3dbpp(a->fbox,oldl));
  memcpy(a->closed, oldclosed, sizeof(boolean_3dbpp)*(bno+1));
}
...
void clearboxes(allinfo *a)
{
  ...
  for (i = a->fbox, m = a->lbox+1; i != m; i++) {
    i->x = i->y = i->z = i->bno = 0; i->k = 0; i->vol = VOL_3dbpp(i);
  }
...
void copyboxes(allinfo *a, int *w, int *h, int *d, int W, int H, int D)
{
  ...
  for (i = a->fbox, m = a->lbox+1, k = 0; i != m; i++, k++) {
    ...
    if ((w[k] < 1) || (w[k] > W)) printf("(3dbpp): bad w\n");
    if ((h[k] < 1) || (h[k] > H)) printf("(3dbpp): bad h\n");
    if ((d[k] < 1) || (d[k] > D)) printf("(3dbpp): bad d\n");
  }
               ...
               int packingtype)
{
  ...
  boolean_3dbpp cl[MAXBOXES];

  /* start the timer */ 
  timer(NULL); stopped = 0; 
 
  /* copy info to a structure */
  if ((packingtype != ROBOT) && 
      (packingtype != GENERAL_3dbpp)) printf("(3dbpp): bad packtype \n");
  if (n+1 > MAXBOXES) printf("(3dbpp): too big instance \n");
