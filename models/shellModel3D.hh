...
#ifndef SHELL_MODEL_3D_HH
#define SHELL_MODEL_3D_HH
...
template<typename T>
ShellModel3D<T>::ShellModel3D (T density_)
    : density(density_)
{ }

template<typename T>
SpringModel3D<T>::SpringModel3D (
        T density_, T k_rest_, T k_stretch_, T k_shear_, T k_bend_ )
...
template<typename T>
Array<T,3> SpringModel3D<T>::computeElasticForce (
        ...
        plint iVertex )
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        iPosition[i] += eps;
        T up = shellModelHelper3D::springModelHelper3D::computePotential (
                iVertex, iPosition, dynMesh, eqMesh, k_rest, k_stretch,
                k_shear, k_bend );
        iPosition = vertex;
        iPosition[i] -= eps;
        T um = shellModelHelper3D::springModelHelper3D::computePotential (
                iVertex, iPosition, dynMesh, eqMesh, k_rest, k_stretch,
                k_shear, k_bend );
