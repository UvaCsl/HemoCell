...
#ifndef INTERMEDIATE_MODEL_3D_HH
#define INTERMEDIATE_MODEL_3D_HH
...
template<typename T, template<typename U> class Descriptor>
plint IntermediateModel3D<T,Descriptor>::getEdgeId(plint iVertex, plint jVertex) {
...
template<typename T, template<typename U> class Descriptor>
void IntermediateModel3D<T, Descriptor>::computeCellForce (Cell3D<T,Descriptor> * cell) {
    ...
    for (pluint iE = 0; iE < edges.size(); ++iE) {
        ...
        SurfaceParticle3D* iParticle = castParticleToICP3D(cell->getParticle3D(iVertex));
        SurfaceParticle3D* jParticle = castParticleToICP3D(cell->getParticle3D(jVertex));
          /* ------------------------------------*/
        ...
        if (angleFound) {
            ...
            SurfaceParticle3D* kParticle = castParticleToICP3D(cell->getParticle3D(kVertex));
            SurfaceParticle3D* lParticle = castParticleToICP3D(cell->getParticle3D(lVertex));
...
#endif  // INTERMEDIATE_MODEL_3D_HH
