...
class FLUID(object):
    def __init__(self, dt=None, dx=None, dxdydz=None, iteration=None, numberofcells=None, processorId=None, relativeposition=None, subdomainsize=None, position=None, velocity=None, boundary=None, shearStress=None):
        self.dt = dt 
        self.dx = dx
        self.dxdydz = dxdydz
...
#Cell class of attributes from the hdf5 files (Can be RBCs, WBCs, Platelets from HemoCell)
...
#The read function
def core_read_processor(f,r,p,ct3,full,half,t,datapath,fluidname,rbcname,pltname,ct3name,n):
    ...
    # Sjon toevoeging
    fb = []     # fluid boundary
    fSS = []    # Shear stress tensor upper triangular half of 3x3 matrix, i.e. 6 values xx xy xz yy yz zz
    ...
    if f:
        ...
        try:
            fluid_file = h5py.File(datapath+str(t)+ "/" + fluidname + "." + str(t) +".p." +str(n) +".h5","r")
        except:
            try:
                fluid_file = h5py.File(time_path+ "/" + fluidname + "." + str(t).zfill(12) +".p." +str(n) +".h5","r")    
        ...
        if half:
            ...
            tempBoundary = np.array(fluid_file["Boundary"])
            tempShearStress = np.array(fluid_file["ShearStress"])
            ...
            xblocks = np.shape(tempBoundary)[2]
            yblocks = np.shape(tempBoundary)[1]
            zblocks = np.shape(tempBoundary)[0]

            for xpos in range(xblocks):
                for ypos in range(yblocks):
                    for zpos in range(zblocks):
                        vel.append(np.array([tempvel[zpos][ypos][xpos][0], \
                                             tempvel[zpos][ypos][xpos][1], \
                                             tempvel[zpos][ypos][xpos][2]]))
                        boundary.append(np.array([tempBoundary[zpos][ypos][xpos][0]]))
                        shearStress.append(np.array(tempShearStress[zpos][ypos][xpos]))
                        pos.append(np.array([xpos+relpos[2],ypos+relpos[1],zpos+relpos[0]]))

            fpos.extend(pos)
            fvel.extend(vel)
            fSS.extend(shearStress)
            fb.extend(boundary)
    ...
    return(fdt,fdx,fdxdydz,fiter,fNpart,fprocid,frelpos,fsubdomsize,fpos,fvel,
        ...
        ct3dt,ct3dx,ct3iter,ct3Npart,ct3Nproc,ct3Ntri,ct3procid,ct3Farea,ct3Fbend,ct3Flink,ct3Ftotal,ct3Fvisc,ct3Fvol,ct3triangles,ct3pos,ct3cid,fb,fSS)



#The wrapper around the read function 
#This will read in a time range and all the run processors
def open_hdf5_files(f=True,r=True,p=True,ct3=True,full=False,half=False,read_procs=1,timesteps=range(0,10000000,5000000),datapath=".",fluidname="Fluid",rbcname="RBC",pltname="PLT",ct3name="RBC_stiff",nprocs=1):
    ...
    for t in timesteps:
        ...
        if read_procs >1:
            ...
            read_function = partial(core_read_processor, f, r, p, ct3, full, half, t, datapath, fluidname, rbcname, pltname, ct3name)
            #Read in parallel
            ...
            for i in range(nprocs):
                ...
                # SJON
                fb.extend(result[i,58])
                fSS.extend(result[i,59])

        elif read_procs == 1:
            #Read in serial if only 1 read processor
            result = core_read_processor(f,r,p,ct3,full,half,t,datapath, \
                                         fluidname,rbcname,pltname,ct3name,0)
        ...
        if f:
            fluid.append(FLUID(fdt,fdx,fdxdydz,fiter,fNpart,fprocid,frelpos,fsubdomsize,fpos,fvel,fb,fSS))
        if r:
