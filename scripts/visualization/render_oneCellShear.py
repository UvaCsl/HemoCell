...
x3dpath = "/mnt/extra_storage/ben/hemocell/examples/oneCellShear/tmp/x3d/"
renderpath = "/mnt/extra_storage/ben/hemocell/examples/oneCellShear/tmp/renders/"
...
def rotate_all(x=0,y=0,z=0):
    for i in range(len(bpy.data.objects)):
        ...
        rot_mat = Matrix.Rotation(radians(x), 4, 'X')   # you can also use as axis Y,Z or a custom vector like (x,y,z)
        ...
        rot_mat = Matrix.Rotation(radians(y), 4, 'Y')   # you can also use as axis Y,Z or a custom vector like (x,y,z)
        ...
        rot_mat = Matrix.Rotation(radians(z), 4, 'Z')   # you can also use as axis Y,Z or a custom vector like (x,y,z)
...
#Create render directory if doesn't exist
if not os.path.isdir(renderpath):
    os.mkdir(renderpath)

#Main time render loop
for time in range(t_0,t_f+timestep,timestep):
    #First Clear the scene of evertyhing
    clearscene()
    #set the render engine
    bpy.context.scene.render.engine = render_engine
    
    #If you want to cell the number of cores devoted to render uncomment
    #for scene in bpy.data.scenes:
    #    scene.render.threads_mode = 'FIXED'
    #    scene.render.threads = 18


    #Import the geometries at the correct timestep
    importx3d(filename=x3dpath+'RBC.'+str(time).zfill(12)+".x3d",objectname = "RBC")

    # Rotate and scale geomtries (this will stongly depend on the view that you want)
    # Scalling, framing, and placing objects llike cells, lights, and cameras are always a hack via this api. It is straigh forward in the GUI
    ...
    #remove duplicate cells since HemoCell can save duplicates when cells are being copied from one core to another
    ...
    #===============================================================================================================
    #Add material for RBC===========================================================================================
    #smooth it
    smooth_mesh("RBC")
    bpy.data.materials.new("RBC")
    bpy.data.meshes["RBC"].materials.append(bpy.data.materials["RBC"])
    #This is all done using NODES. Much easier to understand in GUI
    bpy.data.materials["RBC"].use_nodes =True
    #play around with the color
    bpy.data.materials["RBC"].node_tree.nodes["Principled BSDF"].inputs[0].default_value = (1, 0.9, 0.847222, 1)
    bpy.data.materials["RBC"].node_tree.nodes["Principled BSDF"].inputs[1].default_value = 0.88
    bpy.data.materials["RBC"].node_tree.nodes["Principled BSDF"].inputs[3].default_value = (1, 0, 0, 1)
    bpy.data.materials["RBC"].node_tree.nodes["Principled BSDF"].inputs[5].default_value = 0.16
    bpy.data.materials["RBC"].node_tree.nodes["Principled BSDF"].inputs[7].default_value = 0.67
    #Add some texture to the cell    
    bpy.data.materials["RBC"].node_tree.nodes.new(type="ShaderNodeTexNoise")
    bpy.data.materials["RBC"].node_tree.nodes["Noise Texture"].inputs[3].default_value = 6
    bpy.data.materials["RBC"].node_tree.nodes["Noise Texture"].inputs[2].default_value = 5
    #link RGB curves to vertex colors
    bpy.data.materials["RBC"].node_tree.links.new(bpy.data.materials["RBC"].node_tree.nodes["Noise Texture"].outputs[1],
     bpy.data.materials["RBC"].node_tree.nodes["Material Output"].inputs[2])
    #END ddd material for RBC===========================================================================================
    #===============================================================================================================
    ...
    #==================================================================================================
    #make sure its a white background===============================
    #==================================================================================================
    #This is quite complicated in Blender comment out to remove white background
    ...
    #==================================================================================================    
    #END make sure its a white background===============================
    #==================================================================================================    

    #set the savefilename for the render
    bpy.context.scene.render.filepath = renderpath+final_render_filename+'_'+str(time).zfill(12)+'.png'

    #Turn on denoiser if cycles
