...
def createH5TopologyAndGeometryCell(xmlInt=XMLIndentation()):
    ...
    h5TopologyAndGeometry += xmlInt.inc() + '<Topology TopologyType="Polygon" NodesPerElement="2" NumberOfElements="%(numberOfLines)d">\n'
    h5TopologyAndGeometry += xmlInt.inc() + '<DataItem Dimensions="%(numberOfLines)d 2" Format="HDF">\n'
    h5TopologyAndGeometry += xmlInt.cur() + '%(pathToHDF5)s:/Lines\n'
...
class HDF5toXDMF_Cell(object):
    ...
    def writeSubDomain(self, h5dict, gridName="Subdomain "):
        ...
        for datasetDict in iteratePossibleDataSetsDict(h5dict):
            if not datasetDict['attributeName'] == "Lines":
