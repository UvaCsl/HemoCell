...
public:
    ...
    virtual bool operator() (plint iX, plint iY, plint iZ) const {
        return ((iX-xcircL)*(iX-xcircL) + (iY-ycirc)*(iY-ycirc) <= radiusSqr) ||
               ...
               (iX <= (iY-568.371685741)/(-1.73205080757) && iX >= xcircR && iY <= 155 );
    }
...
//                new BounceBack<T, DESCRIPTOR> );
  ...
  double dpdz = 0.22e5;//(flowQ*12*2.0e-3)/(130e-6*130e-6*130e-6*100e-6); //(shear_rate * 2e-3) / (4*174e-6);
  ...
  vector<int> outputs = {OUTPUT_POSITION,OUTPUT_TRIANGLES,OUTPUT_FORCE,OUTPUT_FORCE_VOLUME,OUTPUT_FORCE_BENDING,OUTPUT_FORCE_LINK,OUTPUT_FORCE_AREA,OUTPUT_FORCE_VISC,OUTPUT_CELL_ID,OUTPUT_VERTEX_ID};
