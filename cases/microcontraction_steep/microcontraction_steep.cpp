...
public:
    ...
    virtual bool operator() (plint iX, plint iY, plint iZ) const {
        return ((iX-xcircL)*(iX-xcircL) + (iY-ycirc)*(iY-ycirc) <= radiusSqr) ||
               ...
               (iX >= xtopL && iX <= xtopR  && iY <= ycirc);
               //(iX <= xtopR && iX >= xcircR && iY <=  );
    }
...
//  plint extendedEnvelopeWidth = 1;  // Because we might use ibmKernel with with 2.
 ...
 // defineDynamics(*hemocell.lattice, *flagMatrix, (*hemocell.lattice).getBoundingBox(), new BounceBack<T, DESCRIPTOR>(1.), 0);
  ...
  double dpdz_lbm = 1.3e-7;//dpdz * ((*cfg)["domain"]["dx"].read<double>() * (*cfg)["domain"]["dx"].read<double>() * (*cfg)["domain"]["dt"].read<double>()*(*cfg)["domain"]["dt"].read<double>() /param::dm);
