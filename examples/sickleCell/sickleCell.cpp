...
//    tmeas = ceil(100.0 * 9.803921568235293e-08/dt);
    ...
    if (minNumOfTriangles <= 42) { shape = 1; minNumOfTriangles = 80; } // Min Number of Vertices
    else if (minNumOfTriangles <= 66) { shape = 4; minNumOfTriangles = 128; }
    else if (minNumOfTriangles <= 162) { shape = 1; minNumOfTriangles = 320; }
    else if (minNumOfTriangles <= 258) { shape = 4; minNumOfTriangles = 512; }
    else if (minNumOfTriangles <= 642) { shape = 1; minNumOfTriangles = 1280; }
    else if (minNumOfTriangles <= 1026) { shape = 4; minNumOfTriangles = 2048; }
    else if (minNumOfTriangles <= 2562) { shape = 1; minNumOfTriangles = 5120; }
}
...
//    Array<T,3> forceRight(0.0, 44.0, 54.0), forceLeft(0.0, -44.0, 54.0);
    Array<T,3> forceRight(44.0, 54.0, 0.0), forceLeft(-44.0, 54.0, 0.0);
    forceLeft *= 1e-12/dNewton; // In picoNewton, in LU
    forceRight *= 1e-12/dNewton; // In picoNewton, in LU
    SickleCellForce3D<T, DESCRIPTOR> sickleCell(RBCField, forceLeft, forceRight, 0.1);
...
//            pcout << "; Volume (" << RBCField[0]->getVolume() << ")";
			...
			Array<T,3> stretch = sickleCell.measureStretch();
    ...
    RBCField[0]->saveMesh("sickleCell.stl");
