...
void RbcMalariaModel::ParticleMechanics(map<int,vector<HemoCellParticle *>> & particles_per_cell, const map<int,bool> & lpc, size_t ctype) {
  ...
  for (const auto & pair : lpc) { //For all cells with at least one lsp in the local domain.
    ...
    for (const hemo::Array<plint,3> & triangle : cellConstants.triangle_list) {
      const hemo::Array<double,3> & v0 = cell[triangle[0]]->sv.position;
      const hemo::Array<double,3> & v1 = cell[triangle[1]]->sv.position;
      const hemo::Array<double,3> & v2 = cell[triangle[2]]->sv.position;
...
//Per-vertex bending force loop
    for (long unsigned int i = 0 ; i < cell.size() ; i++) {
      ...
      const hemo::Array<double,3> dev_vect = vertexes_middle - cell[i]->sv.position;
      ...
      for(unsigned int j = 0; j < cellConstants.vertex_n_vertexes[i]-1; j++) {
        hemo::Array<double,3> triangle_normal = crossProduct(cell[cellConstants.vertex_vertexes[i][j]]->sv.position - cell[i]->sv.position, 
      ...
      hemo::Array<double,3> triangle_normal = crossProduct(cell[cellConstants.vertex_vertexes[i][cellConstants.vertex_n_vertexes[i]-1]]->sv.position - cell[i]->sv.position, 
      ...
      const hemo::Array<double,3> rel_vel_v = vertices_vavg - cell[i]->sv.v;
    ...
    for (const hemo::Array<plint,2> & edge : cellConstants.edge_list) {
      const hemo::Array<double,3> & p0 = cell[edge[0]]->sv.position;
      const hemo::Array<double,3> & p1 = cell[edge[1]]->sv.position;
      ...
      *cell[edge[1]]->force_link -= force;
      ...
      // F = eta * (dv/l) * l. 
      const hemo::Array<double,3> rel_vel = cell[edge[1]]->sv.v - cell[edge[0]]->sv.v;
...
double RbcMalariaModel::calculate_etaV(Config & cfg ){
  return cfg["MaterialModel"]["eta_v"].read<double>() * param::dx * param::dt / param::dm; //== dx^2/dN/dt
};

double RbcMalariaModel::calculate_etaM(Config & cfg ){
  return cfg["MaterialModel"]["eta_m"].read<double>() * param::dx / param::dt / param::df;
};

double RbcMalariaModel::calculate_kBend(Config & cfg, MeshMetrics<double> & meshmetric ){
  double eqLength = meshmetric.getMeanLength();
  return cfg["MaterialModel"]["kBend"].read<double>() * param::kBT_lbm / eqLength;
};

double RbcMalariaModel::calculate_kVolume(Config & cfg, MeshMetrics<double> & meshmetric){
...
double RbcMalariaModel::calculate_kArea(Config & cfg, MeshMetrics<double> & meshmetric){
...
double RbcMalariaModel::calculate_kLink(Config & cfg, MeshMetrics<double> & meshmetric){
