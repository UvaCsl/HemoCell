...
int main(int argc, char* argv[])
{
    ...
    pos.push_back(30); //lmount: pos.push_back(30);pos.push_back(50);
    ...
    for (plint iN = 0; iN < nMax; ++iN) { // create 40 * inlet amount of particles
        for (pluint iA = 0; iA < pos.size(); ++iA) {
            for (pluint iB = 0; iB < pos.size(); ++iB) {
                centers.push_back(Array<T,3>(6+iN,pos[iA],pos[iB]));
                radii.push_back(2);
            }
        }
    }
    ...
    TriangleBoundary3D<T> bloodCells = createCompleteMesh<T>(centers, radii, tags, numPartsPerBloodCell);
...
//             for (pluint iA = 0; iA < centers.size(); ++iA) {
                ...
                writeImmersedSurfaceVTK (
                        ...
                        *getParticlePosAndVelocity(immersedParticles),
                        ...
                        global::directories().getOutputDir()+createFileName("_surfVel",i,6)+".vtk", dynamicMesh, tag );
