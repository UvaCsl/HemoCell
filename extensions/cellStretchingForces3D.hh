        ...
        for (pluint j = 0; j < vec->size(); ++j) {
            ...
            if (tagToParticle3D.count(tag) > 0) {
                SurfaceParticle3D* particle =
                        dynamic_cast<SurfaceParticle3D*> (tagToParticle3D[tag]);
...
bool compareParticlesInX_PBC (Particle3D<T,Descriptor>* iParticle, Particle3D<T,Descriptor>* jParticle) {
    T iX = dynamic_cast<SurfaceParticle3D*> (iParticle)->get_pbcPosition()[0];
    T jX = dynamic_cast<SurfaceParticle3D*> (jParticle)->get_pbcPosition()[0];
    return (iX<jX);
}
...
bool compareParticlesInY_PBC (Particle3D<T,Descriptor>* iParticle, Particle3D<T,Descriptor>* jParticle) {
    T iY = dynamic_cast<SurfaceParticle3D*> (iParticle)->get_pbcPosition()[1];
    T jY = dynamic_cast<SurfaceParticle3D*> (jParticle)->get_pbcPosition()[1];
    return (iY<jY);
}
...
bool compareParticlesInZ_PBC (Particle3D<T,Descriptor>* iParticle, Particle3D<T,Descriptor>* jParticle) {
    T iZ = dynamic_cast<SurfaceParticle3D*> (iParticle)->get_pbcPosition()[2];
    T jZ = dynamic_cast<SurfaceParticle3D*> (jParticle)->get_pbcPosition()[2];
    return (iZ<jZ);
}
